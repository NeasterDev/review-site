...
db.once('open', async () => {
    ...
    for (let i = 0; i < 10; i += 1) {
      ...
      const _id_2 = Types.ObjectId();
      //console.log(_id);
      const username = faker.internet.userName();
      const location = nationalParks[Math.floor(Math.random()*nationalParks.length)];
      const email = faker.internet.email(username);
      const password = faker.internet.password();
      const reviewText = faker.random.words(25);
      const rating = Math.floor(Math.random()*5);
      //const _id = faker.datatype.uuid();
      const createdAt = new Date();
  
      userData.push({ username, email, password, _id, savedReviews: [{location, reviewText, rating, username, _id: _id_2, createdAt, upvotes: 0, downvotes: 0, imageUrls: []}] });
